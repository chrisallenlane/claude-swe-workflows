...
Supporting workflows available at any level: `/think-reframe` (problem redefinition), `/think-brainstorm` (divergent idea generation), `/think-diagnose` (abductive reasoning about causes), `/think-deliberate` (adversarial decision-making), `/think-scrutinize` (adversarial idea critique), `/bug-fix` (diagnosis-first bug fixing), `/bug-hunt` (proactive bug discovery), `/test-mutation` (mutation testing), `/refactor-deep` (full tactical + architectural + tactical refactoring cycle).
